...
import MenuItemReveiwsTable from "main/components/MenuItemReviews/MenuItemReviewsTable";
...
describe("MenuItemReviewsTable tests", () => {
  ...
  test("renders without crashing for empty table with user not logged in", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <MenuItemReveiwsTable menuItemReviews={[]} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>
  ...
  test("Has the expected colum headers and content for adminUser", () => {
    ...
    const { getByText, getByTestId } = render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <MenuItemReveiwsTable menuItemReviews={menuItemReviewsFixtures.threeReviews} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>
  ...
  test("Edit button navigates to the edit page for admin user", async () => {
    ...
    await waitFor(() => { expect(getByTestId(`MenuItemReviewsTable-cell-row-0-col-itemId`)).toHaveTextContent("1"); });
